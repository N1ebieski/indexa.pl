...
class CSVCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'idir:csv {--workers=1} {--partial}';
     ...
     * @var string
     */
    protected $description = 'Import database from CSV';
    ...
    public function handle()
    {
        ...
        $bar = $this->output->createProgressBar($this->option('partial') === false ? 8 : 1);
        
        $this->line("CSV importer");
        $this->line("Author: Mariusz Wysokiński");
        $this->line("Version: {$this->config->get('idir.version')}");
        $this->line("\n");
        if ($this->option('partial') === false) {
            $this->confirmation();
            $this->line("\n");
            $bar->start();
            $this->line("\n");
        ...
        $this->line("\n");
        $this->call('db:seed', ['--class' => 'App\Seeds\CSV\CSVSeeder', '--force' => true], $this->getOutput());
        $this->info("\n");
        $bar->finish();
    }
}
